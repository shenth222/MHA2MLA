...
def load_optimizer_scheduler(model, config):
    """Load learning rate scheduler from configuration."""
    # adamW
    optimizer_cfg = config["Optimizer"]
    optimizer_name = optimizer_cfg["optimizer_factory"]["name"].lower()
    if optimizer_name == "adamw":
        optimizer = torch.optim.AdamW(
            params=model.parameters(),
            lr=optimizer_cfg["learning_rate_scheduler"]["learning_rate"],
            betas=(
                optimizer_cfg["optimizer_factory"]["adam_beta1"],
                optimizer_cfg["optimizer_factory"]["adam_beta2"],
            ),
            eps=optimizer_cfg["optimizer_factory"]["adam_eps"],
            weight_decay=optimizer_cfg["weight_decay"],
            fused=optimizer_cfg["optimizer_factory"]["torch_adam_is_fused"],
        )
    else:
        raise ValueError(
            f"Unknown optimizer factory {optimizer_cfg['optimizer_factory']['name']}"
        )
    # lr_scheduler
    lr_scheduler = lr_scheduler_builder(
        optimizer=optimizer,
        lr_scheduler_args=SimpleNamespace(**optimizer_cfg["learning_rate_scheduler"]),
        total_training_steps=config["TrainingArguments"]["max_steps"],
    )
