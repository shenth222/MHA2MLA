...
def get_prompt_function(task_name):
    """
    尝试从本文件搜索函数，如果没有找到，再从指定模块加载。
    
    :param task_name: 函数名称
    :return: 函数对象
    """
    import importlib
    import sys
    ...
    try:
        ...
        raise ValueError(f"函数 {task_name} 在本文件或模块 'lighteval.tasks.default_prompts' 中未找到: {e}")
...
COMMON_SENSE_REASONING_TASKS = [
    LightevalTaskConfig(
        name="hellaswag",
        prompt_function=get_prompt_function("hellaswag_prompt"),
        hf_repo="hellaswag",
        hf_subset="default",
    ...
    LightevalTaskConfig(
        name="winogrande",
        prompt_function=get_prompt_function("winogrande"),
        hf_repo="winogrande",
    ...
    LightevalTaskConfig(
        name="piqa",
        prompt_function=get_prompt_function("piqa_harness"),
        hf_repo="piqa",
    ...
    LightevalTaskConfig(
        name="siqa",
        prompt_function=get_prompt_function("siqa_prompt"),
    ...
    LightevalTaskConfig(
        name="openbookqa",
        prompt_function=get_prompt_function("openbookqa"),
        hf_repo="openbookqa",
        hf_subset="main",
        metric=[Metrics.loglikelihood_acc, Metrics.loglikelihood_acc_norm_nospace],
    ),
    LightevalTaskConfig(
        name="arc:easy",
        prompt_function=get_prompt_function("arc"),
        hf_repo="ai2_arc",
        hf_subset="ARC-Easy",
    ...
    LightevalTaskConfig(
        name="commonsense_qa",
        prompt_function=get_prompt_function("commonsense_qa_prompt"),
    ...
    LightevalTaskConfig(
        name="mmlu_pro_cloze",
        prompt_function=get_prompt_function("mmlu_pro_cloze_prompt"),
    ...
    LightevalTaskConfig(
        name="boolq",
        prompt_function=get_prompt_function("boolq_prompt"),
        hf_repo="super_glue",
        hf_subset="boolq",
        metric=[Metrics.loglikelihood_acc, Metrics.loglikelihood_acc_norm_nospace],
        trust_dataset=True,
    ...
    LightevalTaskConfig(
        name="trivia_qa",
        prompt_function=get_prompt_function("triviaqa"),
...
MATH_TASKS = [
    LightevalTaskConfig(
        name=f"math:{subset}",
        prompt_function=get_prompt_function("math"),
    ...
    for subset in mmlu_subsets:
        MMLU_TASKS.append(
            CustomMMLUEvaluationTask(
                ...
                prompt_function=get_prompt_function(prompt_function),
                hf_subset=subset,
                generation_size=generation_size
            )
        )
...
MMLU_TASKS += [
    CustomMMLUEvaluationTask(
        ...
        prompt_function=get_prompt_function("mmlu_mc_prompt"),
        hf_subset="default",
        generation_size=1
    ),
    CustomMMLUEvaluationTask(
        ...
        hf_repo="TIGER-Lab/MMLU-STEM",
        prompt_function=get_prompt_function("mmlu_cloze_prompt"),
        hf_subset="default",
        generation_size=-1
    ),
]
