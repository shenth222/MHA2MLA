...
def HDM_for_weight(model, mode="Q", rank=32):
    ...
    def get_weights(layer_idx):
        if mode == "Q":
            ...
            return model.model.layers[layer_idx].mlp.up_proj.weight.detach()
        elif mode == "D":
            return model.model.layers[layer_idx].mlp.down_proj.weight.detach().T
        else:
            return model.model.layers[layer_idx].mlp.gate_proj.weight.detach()

    
    tbar = tqdm(range(num_layers))
    for layer_idx in range(num_layers):
        tbar.update(1)
        tbar.set_description("Computing layer {layer_idx+1}")
        tbar.refresh()
...
def save_HDM_res(W1, H1, W2, H2, save_path="./HDM_res", mode="Q"):
...
def load_HDM_res(load_path="./HDM_res", mode="Q", rank=32):
    ...
    path = f"{load_path}/H1_{mode}_{RANK}.pkl"
    H1 = torch.load(path, weights_only=True)
    path = f"{load_path}/W2_{mode}_{RANK}.pkl"
    W2 = torch.load(path, weights_only=True)
    path = f"{load_path}/H2_{mode}_{RANK}.pkl"
...
if __name__ == "__main__":
    ...
    save_HDM_res(W1, H1, W2, H2, mode=mode)
