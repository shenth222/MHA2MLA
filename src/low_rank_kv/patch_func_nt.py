...
class CustomCausalSelfAttention(nn.Module, AttachableStore):
    ...
    def get_key_states(
        ...
        k_r = self.W_k_r(hidden_states)  # [seq_length, batch_size, rope_dim]
        c_k = self.W_down_k(self.k_proj(hidden_states))  # [seq_length, batch_size, low_rank * n_local_kv_heads]
        k_c = self.W_up_k(apply_activation(c_k,self.config.SVD["activation_fn"]))  # [seq_length, batch_size, nope_dim]
...
def custom_load_weights(
    ...
    for missing_key in missing_keys:
        ...
        if missing_key.endswith("W_down_k.weight"):
            ...
            U,S,V = torch.svd(torch.eye(attn_module.nope_mask.sum().item()).to(dtype=torch.float32))
            U = U[:,:low_rank].to(dtype=dtype)
            S = S[:low_rank].to(dtype=dtype)
            V = V[:,:low_rank].to(dtype=dtype)
            filtered_state_dict[f"{attn_module_prefix}.W_k_r.weight"]=(k_proj[:,~attn_module.nope_mask]).T
            filtered_state_dict[f"{attn_module_prefix}.k_proj.weight"]=(k_proj[:,attn_module.nope_mask]).T
...
def low_rank_kv_patch_func_nt(rope_cfg=None):
