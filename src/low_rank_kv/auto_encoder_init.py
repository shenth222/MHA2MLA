...
class AttnForTraing(PreTrainedModel):
    ...
    def post_init(self,original_model):
        ...
        for layer_idx, layer in enumerate(self.original_model.model.layers):
            ...
            target_attn.q_proj.weight.data[:] = q_proj
            target_attn.k_proj.weight.data[:] = (k_proj.T[:, target_attn.nope_mask].T)
            target_attn.v_proj.weight.data[:] = v_proj
            target_attn.o_proj.weight.data[:] = original_attn.o_proj.weight.detach()
            target_attn.W_k_r.weight.data[:] = (k_proj.T[:, ~target_attn.nope_mask].T)
            # W_down_k, W_up_k
            U,S,V = torch.svd(torch.eye(target_attn.nope_mask.sum().item()).to(dtype=torch.float32))
            low_rank = self.config.SVD["low_rank"] * self.config.num_key_value_heads
            dtype = target_attn.W_down_k.weight.dtype
            U = U[:,:low_rank].to(dtype=dtype)
            S = S[:low_rank].to(dtype=dtype)
            V = V[:,:low_rank].to(dtype=dtype)
            target_attn.W_down_k.weight.data[:] = U.t()
            target_attn.W_up_k.weight.data[:] = (torch.diag(S) @ V.t()).t()
            target_attn.W_down_k.weight.requires_grad_(True)
            target_attn.W_up_k.weight.requires_grad_(True)
            # W_down_v, W_up_v
            U,S,V = torch.svd(torch.eye(original_attn.num_key_value_heads * original_attn.head_dim).to(dtype=torch.float32))
            low_rank = self.config.SVD["low_rank"] * self.config.num_key_value_heads
            dtype = target_attn.W_down_v.weight.dtype
            U = U[:,:low_rank].to(dtype=dtype)
            S = S[:low_rank].to(dtype=dtype)
            V = V[:,:low_rank].to(dtype=dtype)
            target_attn.W_down_v.weight.data[:] = U.t()
            target_attn.W_up_v.weight.data[:] = (torch.diag(S) @ V.t()).t()
            target_attn.W_down_v.weight.requires_grad_(True)
            target_attn.W_up_v.weight.requires_grad_(True)
    ...
    def forward(
        ...
        low_rank_kv_patch_func_hf(self.config.RoPE)
        for layer_idx,layer in enumerate(self.original_model.model.layers):
            original_attn = layer.self_attn
            nope_mask = self.model[layer_idx].nope_mask
            hidden_states = self.inputs[layer_idx][1]["hidden_states"]
            original_key_states = original_attn.k_proj(hidden_states)
            original_value_states = original_attn.v_proj(hidden_states)
            _,key_states,value_states = self.model[layer_idx].get_qkv_states(
                *self.inputs[layer_idx][0],
                **self.inputs[layer_idx][1],
            )
            key_states = key_states.transpose(1,2).view(*original_key_states.shape)
            value_states = value_states.transpose(1,2).view(*original_value_states.shape)
            k_loss = self.loss_func(original_key_states[...,nope_mask], key_states[...,nope_mask])
