...
class CustomLlamaAttention(nn.Module):
    ...
    def get_qkv_states(
        ...
        **kwargs,
        ...
        c_kv = torch.cat(
            [
                self.W_down_k(self.k_proj(hidden_states)),
                self.W_down_v(self.v_proj(hidden_states)),
            ],
            dim=-1,
        )
        c_kv = apply_activation(c_kv, self.config.SVD["activation_fn"])
...
def low_rank_kv_patch_func_hf(rope_cfg=None):
