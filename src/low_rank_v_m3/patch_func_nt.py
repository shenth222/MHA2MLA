...
def custom_load_weights(
    ...
    for missing_key in missing_keys:
        ...
        if missing_key.endswith("W_down_k.weight"):
            ...
            # assert torch.any(filtered_state_dict[f"{attn_module_prefix}.W_down_v.weight"] != 0), "W_down_v.weight 矩阵全为 0"
            # assert torch.any(filtered_state_dict[f"{attn_module_prefix}.W_up_v.weight"] != 0), "W_up_v.weight 矩阵全为 0"
            # dtype = filtered_state_dict[f"{attn_module_prefix}.W_down_v.weight"].dtype
